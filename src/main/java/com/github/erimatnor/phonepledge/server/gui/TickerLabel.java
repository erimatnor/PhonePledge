...
public class TickerLabel extends JComponent {
    ...
    private final TickerTimeout tickerTimeout = new TickerTimeout();
    private volatile int currOffset[] = { 0, 0 };
    // Fraction of the screen width to move every step
	private int ticksPerSecond = 100;
	private BufferedImage tickerLogo = null;
	private Image tickerLogoScaled = null;
	private ImageIcon icon;
	private JPanel[] p = new JPanel[2];
	private int pIndex = 0;
	private int pIndexOld = 1;
	...
	private int width = 0;
	private volatile int tickerHz = 20;
	...
	public TickerLabel(String[] newText) {
		...
		try {
			if (logoFile == null || !logoFile.exists()) {
                tickerLogo = ImageIO.read(ClassLoader.getSystemResourceAsStream("pp-logo-small.png"));
            } else {
	...
	private boolean nextText() {
        ...
        for (final String aText : text) {
            final JLabel l = new JLabel(aText);
            l.setIcon(icon);
            l.setFont(font);
            l.setForeground(tickerFontColor);
            l.setIconTextGap(5);
            l.setOpaque(false);
            int width = metrics.stringWidth(aText) +
	...
	private class TickerTimeout implements Runnable {
